...
en_asset_records = []  # (line_idx, cmd, speaker, text_field, full_line)
...
# Build reverse map: normalized EN novel -> JP key
def normalize_for_match(text):
    ...
    # Remove tags
    text = re.sub(r'<[^>]+>', '', text)
    # Normalize fullwidth comma to ascii
...
print(f"EN novel entries (non-empty): {len(en_novel_to_jp)}")

# Also build ja.json reverse for title (ja.json is identity map for JP)
# The title in EN asset is at line 19 (0-indexed), field 1 is the JP title
title_line_idx, title_cmd, title_speaker, title_text, title_full = en_asset_records[0]
print(f"Title record: line={title_line_idx}, speaker='{title_speaker}', text='{title_text}'")

# For each asset record, find matching JP key and translate to VI
vi_translations = {}  # asset_text_field -> vi_text

# Full translation map from JP to VI
JP_TO_VI = {
    ...
    "しかもすごい大雨だな！？<br>急いで建物に入れ！　風邪を引くぞ！": "Mưa to lắm!?<br>Nhanh vào nhà! Sẽ cảm (ốm) đấy!",
    "は、はいっ！": "Vâng, vâng!",
...
# Match each asset record
for line_idx, cmd, speaker, text_field, full_line in en_asset_records:
    # Special handling for title (JP in speaker field, text_field empty)
    if cmd == 'title,' and not text_field and speaker:
        jp_text = speaker
        if jp_text in JP_TO_VI:
            vi_translations[text_field] = JP_TO_VI[jp_text]
            print(f"Title matched: {jp_text} -> {JP_TO_VI[jp_text]}")
        else:
            print(f"Title NO MATCH: {jp_text}")
        continue
    
    # For message etc, match text_field to EN novel
    ...
    if norm_asset in en_novel_to_jp:
        ...
        for en_norm, jp in en_novel_to_jp.items():
            ...
            if norm_asset in en_norm or en_norm in norm_asset:
                # Verify significant overlap
                if len(set(norm_asset.split()) & set(en_norm.split())) > 5:
                    jp_key = jp
                    break
    ...
    if jp_key and jp_key in JP_TO_VI:
        ...
        print(f"JP key found but no VI translation: {jp_key[:60]}")
    else:
        print(f"NO MATCH line {line_idx} ({cmd}): speaker={speaker} text={text_field[:80]}")

print(f"\nTotal VI translations: {len(vi_translations)}")

# Now rebuild the VI asset file
vi_lines = list(en_asset_lines)
replaced = 0
for line_idx, cmd, speaker, text_field, full_line in en_asset_records:
    if text_field in vi_translations:
        vi_text = vi_translations[text_field]
        # Replace text_field with vi_text in the line
        parts = full_line.split(',', 5)
        if len(parts) >= 3:
            parts[2] = vi_text
            vi_lines[line_idx] = ','.join(parts)
            replaced += 1
    elif cmd == 'title,' and speaker in JP_TO_VI:
        # Title case: speaker has JP, text_field empty
        vi_text = JP_TO_VI[speaker]
        parts = full_line.split(',', 5)
        if len(parts) >= 2:
            parts[1] = vi_text
            vi_lines[line_idx] = ','.join(parts)
...
# Check for ASCII commas in VI text fields
comma_issues = []
for line_idx, cmd, speaker, text_field, full_line in en_asset_records:
    if text_field in vi_translations:
        vi_text = vi_translations[text_field]
        if ',' in vi_text and not vi_text.startswith('['):  # Ignore placeholder markers
            comma_issues.append((line_idx, cmd, vi_text))

if comma_issues:
    print(f"\nASCII COMMA ISSUES ({len(comma_issues)}):")
    for idx, cmd, txt in comma_issues:
