...
# Load EN novel translation (en.json) - maps JP -> EN
...
# ja.json keys are the JP source texts in order
jp_keys = list(ja_data.keys())
print(f"JA keys: {len(jp_keys)}")

# Verify counts match
assert len(en_asset_records) == len(jp_keys), f"Count mismatch: asset={len(en_asset_records)}, ja={len(jp_keys)}"
...
JP_TO_VI = {
    ...
    "……実は、龍神さまは大変気まぐれなのです。": "...Thực ra, Thần Long rất变덕 (bàn bạc không định).",
...
# Match by sequence order (both have 110 records)
vi_translations = {}
matched = 0
for i, (line_idx, cmd, speaker, text_field, full_line) in enumerate(en_asset_records):
    jp_key = jp_keys[i]
    
    if jp_key in JP_TO_VI:
        vi_text = JP_TO_VI[jp_key]
        
        # Preserve trailing <br> from asset text_field
        ...
        print(f"NO VI TRANSLATION for seq {i}: {jp_key[:60]}")

print(f"\nMatched by sequence: {matched}")

# Also handle title separately (text_field is empty, JP in speaker)
title_jp = en_asset_records[0][2]
if title_jp in JP_TO_VI:
    vi_translations[''] = JP_TO_VI[title_jp]  # empty string key for title
    print(f"Title: {title_jp} -> {JP_TO_VI[title_jp]}")
...
for line_idx, cmd, speaker, text_field, full_line in en_asset_records:
    if cmd == 'title,':
        ...
        if '' in vi_translations:
            vi_text = vi_translations['']
...
for line_idx, cmd, speaker, text_field, full_line in en_asset_records:
    if cmd == 'title,':
        if '' in vi_translations:
            en_br = count_br(text_field)  # 0 for title
            vi_br = count_br(vi_translations[''])
    else:
        if text_field in vi_translations:
