...
# Read EN asset file (binary to preserve BOM and exact line endings)
...
# Decode to text (preserving exact line endings)
text = en_asset_bytes.decode('utf-8-sig')

# Split lines KEEPING line endings exactly as they are
lines = text.splitlines(keepends=True)

# Text commands to translate
text_cmds = ('title,', 'message,', 'messageTextUnder,', 'messageTextCenter,')

# Normalize function for matching EN novel -> JP
def norm(s):
    s = s.replace('，', ',').replace('、', ',')
    s = re.sub(r'<[^>]+>', '', s)
    s = re.sub(r'\s+', '', s)
...
# Build EN asset text field mapping: (line_idx, cmd, text_field)
asset_texts = []
...
# Build reverse mapping: EN (normalized) -> JP key
en_to_jp = {}
for jp_key, en_val in en_novel.items():
    if en_val:
        en_to_jp[norm(en_val)] = jp_key

# Vietnamese translations for each JP key
jp_to_vi = {
    ...
    "え？": "Ẻ?",
    "ここは戦場の最前線。手柄を取るチャンスはそこかしこに転がっている。<br>ここで手柄をあげて家族に自分が兵士に相応しいと認めさせてやるといい。": "Đây là tiền tuyến chiến trường. Cơ hội lập công đến处处都有.<br>Cậu lập công ở đây rồi bắt gia đình nhận ra cậu xứng đáng làm binh sĩ đi.",
    ...
    "……というわけで、大穴内の掘削作業は現在、難航しております。<br>専門技術を持った工兵隊には期待していたのですが……": "……Vì vậy, công tác khai thác trong Đại Huyệt hiện đang gặp khó khăn.<br>Đội Kỹ Sư có kỹ thuật chuyên môn本来很受期待, nhưng……",
    ...
    "……認めた相手の言うことしか聞かないってところか。<br>職人気質な連中ってのは、こういう時厄介だな。": "……Chỉ nghe theo người mình công nhận thôi à.<br>Những kẻ có tâm nghề lúc này thật khó xử.",
...
print(f'Asset text records: {len(asset_texts)}')

# Now replace text fields in lines
replaced = 0
for line_idx, cmd, asset_text in asset_texts:
    ...
    if jp_key and jp_key in jp_to_vi:
        ...
        ending = lines[line_idx][len(raw):]
        if cmd == 'title,':
            parts = raw.split(',', 1)
            if len(parts) >= 2:
                new_line = parts[0] + ',' + vi_text + ending
...
# Join lines (they already have their original line endings)
...
output_dir = 'E:/AgentTranslation/Translation/vi/RedirectedResources/assets/unnamed_assetbundle/'
os.makedirs(output_dir, exist_ok=True)
with open(os.path.join(output_dir, 'hmn_10500100001.txt'), 'wb') as f:
    f.write(output_bytes)
print(f'Copied to {output_dir}hmn_10500100001.txt')