...
# ja.json keys in order (109 entries)
jp_keys = list(ja_data.keys())
print(f"JA keys: {len(jp_keys)}")

# Full JP -> VI translation map (matching the 109 JP keys exactly)
JP_TO_VI = {
...
# Build VI translations for each asset record by sequential matching
vi_translations = {}  # text_field -> vi_text
asset_vi_map = {}  # line_idx -> vi_text (for title: speaker field)

# Match sequentially: asset records 0-108 match ja.json 0-108
# Asset record 109 (last) has no JP match - it's the final narration
for seq, (line_idx, cmd, speaker, text_field, full_line) in enumerate(en_asset_records):
    if seq < len(jp_keys):
        ...
        if jp_key in JP_TO_VI:
            ...
            if cmd == 'title,':
                # Title: JP in speaker field, replace speaker
                asset_vi_map[line_idx] = ('title', vi_text)
            else:
                # Message: replace text_field
                ...
                asset_vi_map[line_idx] = ('message', text_field, vi_text)
        else:
            print(f"WARNING: No VI translation for JP key at seq {seq}: {jp_key[:50]}")
    else:
        # Last record (seq 109) - no JP match
        # This is the final narration: "—Rumble，rumble，rumble."
        # Translate based on context
        if text_field == "—Rumble，rumble，rumble.<br> ":
            vi_text = "——Gù gù gù.<br> "
            vi_text = vi_text.replace(',', '\u201a')
            asset_vi_map[line_idx] = ('message', text_field, vi_text)
        print(f"Extra record at seq {seq}: {cmd} speaker={speaker} text={text_field[:50]}")

print(f"\nMapped {len(asset_vi_map)} records")

# Apply translations to EN asset lines
vi_lines = list(en_asset_lines)
replaced = 0

for line_idx, record_type, *rest in asset_vi_map.values():
    if record_type == 'title':
        vi_text = rest[0]
        full_line = en_asset_lines[line_idx]
...
for line_idx, cmd, speaker, text_field, full_line in en_asset_records:
    if line_idx in asset_vi_map:
        record_type = asset_vi_map[line_idx][0]
        if record_type == 'title':
            vi_text = asset_vi_map[line_idx][1]
            en_br = count_br(text_field)
            vi_br = count_br(vi_text)
        else:
            vi_text = asset_vi_map[line_idx][2]
            en_br = count_br(text_field)
            vi_br = count_br(vi_text)
        if en_br != vi_br:
