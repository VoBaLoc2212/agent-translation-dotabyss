...
# Normalize text for matching (strip tags, normalize whitespace/commas)
def normalize_for_match(text):
    if not text:
        return ''
    # Remove all tags
    text = re.sub(r'<[^>]+>', '', text)
    # Normalize fullwidth comma and U+201A to ASCII comma
...
for jp_key, en_val in en_data.items():
    if en_val:
        norm = normalize_for_match(en_val)
        if norm:
            en_novel_to_jp[norm] = jp_key

print(f"EN novel entries (non-empty, normalized): {len(en_novel_to_jp)}")

# Also build map from normalized JP key -> JP key (for title which is still JP in asset)
# ja.json is identity map for this file, so JP keys are the same
# Title handling: EN asset has JP title in speaker field (field 1), text_field is empty
# We'll handle title separately
...
JP_TO_VI = {
    ...
    "ほぼ２日間、缶詰になってしまったな……<br>せっかく早く終わったし、２人で外出して羽を伸ばすか？": "Gần hai ngày bị nhốt trong phòng dire dire...<br>Đã xong sớm thế, hai người ta ra ngoài giải tỏa một chút đi?",
    ...
    "ずんぐりむっくりしているドラゴンとは違うのですー！": "Khác hẳn những con Dragon bùn bã, mập mạp!",
...
# For each asset record, find matching JP key and translate
vi_translations = {}  # asset_text_field -> vi_text (with BR preserved)
matched = 0
unmatched = []

for line_idx, cmd, speaker, text_field, full_line in en_asset_records:
    if cmd == 'title,':
        # Title: JP is in speaker field, text_field is empty
        jp_title = speaker
        if jp_title in JP_TO_VI:
            vi_translations[text_field] = JP_TO_VI[jp_title]  # key by empty string for title
            matched += 1
        else:
            unmatched.append((line_idx, cmd, speaker, text_field, "TITLE NO MATCH"))
        continue
    
    # For message records, match by normalized text content
    norm_asset = normalize_for_match(text_field)
    ...
    if norm_asset in en_novel_to_jp:
        ...
        # Try fuzzy: find EN novel entry that matches closely
        for en_norm, jp in en_novel_to_jp.items():
            # Check if they're essentially the same content
            if en_norm == norm_asset:
                jp_key = jp
                break
            # Partial match: one contains the other (for truncated cases)
            if len(en_norm) > 20 and len(norm_asset) > 20:
                if en_norm in norm_asset or norm_asset in en_norm:
                    jp_key = jp
                    break
    ...
    if jp_key and jp_key in JP_TO_VI:
        ...
        # Preserve <br> tags from asset text_field
        # Count <br> in asset text and ensure VI has same count
        asset_br_count = text_field.count('<br>')
        vi_br_count = vi_text.count('<br>')
        
        if asset_br_count != vi_br_count:
            # Need to adjust VI text to match BR count
            # Strategy: if asset has trailing <br> , ensure VI ends with <br>
            # For now, just note the mismatch
            pass
        
        # Also preserve trailing <br> if present in asset
        if text_field.endswith('<br> ') or text_field.endswith('<br>'):
            if not vi_text.endswith('<br>') and not vi_text.endswith('<br> '):
                vi_text = vi_text.rstrip() + '<br> '
        
        vi_translations[text_field] = vi_text
        matched += 1
    else:
        unmatched.append((line_idx, cmd, speaker, text_field[:80]))

print(f"\nMatched: {matched}")
print(f"Unmatched: {len(unmatched)}")
for idx, cmd, speaker, txt in unmatched[:10]:
    print(f"  Line {idx} ({cmd}): speaker={speaker} text={txt}")
if len(unmatched) > 10:
    print(f"  ... and {len(unmatched) - 10} more")

# Now rebuild VI asset
...
for line_idx, cmd, speaker, text_field, full_line in en_asset_records:
    if cmd == 'title,':
        jp_title = speaker
        if jp_title in JP_TO_VI:
            en_br = count_br(text_field)  # text_field is empty for title
            vi_br = count_br(JP_TO_VI[jp_title])
    else:
        if text_field in vi_translations:
            en_br = count_br(text_field)
            vi_br = count_br(vi_translations[text_field])
            if en_br != vi_br:
                br_mismatches.append((line_idx, cmd, en_br, vi_br, text_field[:50], vi_translations[text_field][:50]))

if br_mismatches:
    print(f"\nBR MISMATCHES ({len(br_mismatches)}):")
    for idx, cmd, en_br, vi_br, en_txt, vi_txt in br_mismatches:
...
# Check ASCII commas in VI text fields
comma_issues = []
for line_idx, cmd, speaker, text_field, full_line in en_asset_records:
    if cmd == 'title,':
        jp_title = speaker
        if jp_title in JP_TO_VI:
            vi_text = JP_TO_VI[jp_title]
            if ',' in vi_text and '、' not in vi_text and '，' not in vi_text:
                comma_issues.append((line_idx, cmd, vi_text))
    else:
        if text_field in vi_translations:
            vi_text = vi_translations[text_field]
            if ',' in vi_text and '、' not in vi_text and '，' not in vi_text:
                # Check if comma is inside text (not delimiter) - all our commas should be U+201A
                comma_issues.append((line_idx, cmd, vi_text))
