...
with open('E:/AgentTranslation/dotabyss-translation-main/translations/novels/hmn_10500100002/ja.json', 'r', encoding='utf-8-sig') as f:
    ja_pairs = json.load(f, object_pairs_hook=list)
...
with open('E:/AgentTranslation/dotabyss-translation-main/translations/novels/hmn_10500100002/en.json', 'r', encoding='utf-8-sig') as f:
    en_pairs = json.load(f, object_pairs_hook=list)
...
# Extract asset text commands in order
asset_text_cmds = []  # list of (line_idx, cmd_type, text_field, full_line)
for i, line in enumerate(asset_lines):
    if line.startswith('title,'):
        parts = line.split(',', 1)
        if len(parts) >= 2:
            tf = parts[1].rstrip('\r\n')
            asset_text_cmds.append(('title', i, tf, line))
    elif line.startswith('message,'):
        parts = line.split(',', 2)
        if len(parts) >= 3:
            tf = parts[2].rstrip('\r\n')
            asset_text_cmds.append(('message', i, tf, line))
    elif line.startswith('messageTextUnder,'):
        parts = line.split(',', 2)
        if len(parts) >= 3:
            tf = parts[2].rstrip('\r\n')
            asset_text_cmds.append(('messageTextUnder', i, tf, line))
    elif line.startswith('messageTextCenter,'):
        parts = line.split(',', 2)
        if len(parts) >= 3:
            tf = parts[2].rstrip('\r\n')
            asset_text_cmds.append(('messageTextCenter', i, tf, line))

print(f"Asset text commands: {len(asset_text_cmds)}")
print(f"JA pairs: {len(ja_pairs)}")
print(f"EN pairs: {len(en_pairs)}")

# Build EN JSON value -> JP key mapping (normalize for matching)
def normalize(s):
    # Remove <br> tags, whitespace, fullwidth commas
    s = re.sub(r'<br\s*/?>', '', s)
    s = s.replace('，', ',')
    s = re.sub(r'\s+', '', s)
    return s
...
en_to_jp = {}
for jp_key, en_val in en_pairs:
    if en_val:  # skip empty (title)
        en_to_jp[normalize(en_val)] = jp_key

print(f"EN->JP mapping entries: {len(en_to_jp)}")

# Build JP -> VI translation dictionary (from my translations)
# I'll create this programmatically from the JP texts
jp_texts = [v for k, v in ja_pairs]
print(f"JP texts: {len(jp_texts)}")

# My translations for all 101 JP texts (index 0-100)
vi_translations = {
    # 0: Title
    ...
    "（さっきのベティの顔……あれは、何かを確信している表情だった。<br>だとしたら、きっと何か考えがあるはずだ）": "(Mặt Betty lúc nãy…… đó là biểu情 của sự chắc chắn.<br>Nếu vậy, chắc chắn có kế hoạch.)",
    ...
    "商人さんとは、消耗品や食材を仕入れる時に顔見知りになっていたであります！<br>主計兵の经验が意外なところで活きたであります！": "Với thương nhân, khi nhập vật tư thực phẩm thành quen ạ!<br>Kinh nghiệm quân nhu nơi bất ngờ phát huy ạ!",
...
missing = []
for jp in jp_texts:
    if jp not in vi_translations:
        missing.append(jp)
if missing:
    print(f"MISSING translations for {len(missing)} JP texts:")
    for m in missing:
        print(f"  {m[:60]}...")
else:
    print("All JP texts have translations!")

# Now process each asset text command
# For each, normalize the EN text field, find in en_to_jp, get JP key, then VI translation
updated_lines = asset_lines[:]
unmatched = []

for cmd_type, line_idx, text_field, full_line in asset_text_cmds:
    norm_tf = normalize(text_field)
    jp_key = en_to_jp.get(norm_tf)
    if jp_key and jp_key in vi_translations:
        vi_text = vi_translations[jp_key]
        # Replace text field in the line
        # Need to preserve the exact structure
        if cmd_type == 'title':
            parts = full_line.split(',', 1)
            if len(parts) >= 2:
                # Replace text field, keep line ending
                line_ending = full_line[len(full_line.rstrip('\r\n')):]
                new_line = parts[0] + ',' + vi_text + line_ending
                updated_lines[line_idx] = new_line
        else:
            # message, messageTextUnder, messageTextCenter
            parts = full_line.split(',', 2)
            if len(parts) >= 3:
                line_ending = full_line[len(full_line.rstrip('\r\n')):]
                # Rebuild: cmd,speaker,vi_text + rest
                # But we need to keep parts[3:] if any
